...
The eight-member shoal swims independently of casting. Pike patrol shallower water and have faster, stronger bursts; zander favor deeper presentation and commit less readily. Fish can approach, nibble, reject and cool down without hooking. Once hooked, random horizontal/vertical runs pull against the reel. Holding retrieves; releasing lets the brake pay out under load. Tension fatigues the fish but accumulates strain above 0.9; sustained excess breaks the line. More than four seconds of large slack loses the hook. Land only when the fish is near, shallow, tired and being retrieved. Award fillet portions once, update best, save, then open the catch card. There is no rod destruction, separate lure inventory, fish-release screen or persistent fish population in v1.
...
`fishing-art.js` uses the shared `poseFrames` grip/boot anchors, keeping the physics tip and the visible rod attached across all eight frames. Do not add a second rod to the raster poses. Hooked fish illustrations anchor their mouth near the lure; free fish are fainter below water. `game.js` captures the primary pointer so release outside the canvas still casts/stops reeling. Blur, visibility, lost capture and modal opening cancel a charging throw or release a loaded reel; they never cause an accidental cast. Secondary pointers cannot release another pointer's charge. The canvas's actual CSS rectangle converts pointer positions to logical coordinates. The catch HUD remains semantic DOM/parchment, localized EN/FI, outside world projection.

Regression coverage: normal natural casts for both species and empty/nibble outcomes; 40 seeded controlled fights; over-tension break, slack escape, sinking/slack retrieval, both-species trophy/award/save flow, movement/equipment/travel cancellation, fixed-step equivalence, all eight grip/tip transforms, pointer release outside canvas, scaled hit coordinates, modal pause and catch-card rendering. Audio tests verify splash/nibble/hook/brake/retrieve/snap/catch events, mute and no repeated-clock stacking. Native Canvas screenshots review ready, charge atlas, wet line and a naturally hooked fish. DOM tests are not actual browser layout/animation verification; Web Audio scheduling tests are not subjective listening tests.
