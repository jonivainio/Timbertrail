...
const fixtures=[
 ...
 {id:'window',name:'Open / close the shutters',x:740,box:[688,143,89,98]},
...
function fade(e,dt){const tr=e.transition;tr.time+=Math.min(dt,.05);if(tr.time>=.45&&!tr.switched){tr.switched=true;const h=home(e);h.inside=tr.room;if(tr.sleep){const s=e.state,p=s.player,elapsed=(1-s.dayTime+.28)*900;s.day++;s.dayTime=.28;e.dailyWeather();advanceFire(h.fire,elapsed);for(const f of s.structures)advanceFire(f,elapsed);for(const r of Object.values(s.regions))for(const f of r.structures||[])advanceFire(f,elapsed);p.energy=100;p.warmth=Math.max(p.warmth,65);p.health=clamp(p.health+30,0,100);p.hunger=Math.max(1,p.hunger-12);p.thirst=Math.max(1,p.thirst-15);e.notify('Morning light fills the cabin. Kajo is still dozing.');}else if(tr.room)h.x=camera.min;else{e.state.player.x=root.PERegions.cabin.x;e.state.dog.x=e.state.player.x-45;e.state.dog.mode='sit';}e.emit('change');}if(tr.time>=1){e.transition=null;e.emit('save');}}
...
function interact(e,id){const h=home(e),o=fixtures.find(v=>v.id===id);if(!h.inside||!o||e.transition||!visible(e,id))return false;e.roomTarget=null;e.state.player.moving=false;
 ...
 if(['hearth','kitchen','bed','chest'].includes(id)){e.sound(id==='chest'?'chestOpen':id==='bed'?'cloth':'wood',.35);e.emit('panel',{panel:id==='kitchen'?'home-hearth':'home-'+id});return true;}
 if(id==='window'){h.shutters=!h.shutters;e.sound('shutter',.35);}
...
function transfer(e,id,direction,all=false){const h=home(e),s=e.state,item=root.PE.items[id],max=root.PE.toolDurability[id];if(!h.inside||!item||!visible(e,'chest')||!['store','take'].includes(direction))return false;
 if(direction==='store'){const used=Object.keys(root.PE.items).reduce((n,k)=>n+count(h,k),0),n=max?1:Math.min(all?s.inventory[id]:1,200-used);if(!Number.isInteger(n)||n<1||s.inventory[id]<n||used+n>200)return false;if(max){if(!Number.isFinite(s.toolDurability[id])||s.toolDurability[id]<=0)return false;h.tools[id]||=[];h.tools[id].push(Math.min(max,s.toolDurability[id]));s.inventory[id]--;s.toolDurability[id]=s.inventory[id]?max:0;if(!s.inventory[id]&&s.equipped===id)s.equipped=null;}else{s.inventory[id]-=n;h.storage[id]=(h.storage[id]||0)+n;}}
 else{const n=max?1:all?count(h,id):Math.min(1,count(h,id));if(n<1||!count(h,id))return false;if(max){if(s.inventory[id])return e.notify('Store the matching tool first to preserve its condition.');s.inventory[id]=1;s.toolDurability[id]=h.tools[id].shift();}else{h.storage[id]-=n;s.inventory[id]+=n;}}
...
function action(e,id){const h=home(e),s=e.state;if(!h.inside||e.transition)return false;
 ...
 if(!visible(e,'hearth')&&!visible(e,'kitchen'))return false;
...
function panel(e,type,art,name){const h=home(e),s=e.state,T=x=>root.L?.text(x)||x;
 ...
 return `<div class="home-panel"><h3>${T(h.fire.lit?'A steady warmth':'Cold embers')} · ${Math.ceil(h.fire.fuel)} s</h3><p>${T('Add wood to cook and warm the cabin. The chimney smokes while the fire burns.')}</p><div class="camp-options">${['wood','firewood'].map(id=>`<button data-home-action="${id}" ${s.inventory[id]>0&&h.fire.fuel<720?'':'disabled'}>${art(id)}${name(id)} · ${s.inventory[id]}<small>+${id==='wood'?45:150} s</small></button>`).join('')}${['rawMeat','rawFish'].map(id=>`<button data-home-action="${id}" ${h.fire.lit&&s.inventory[id]>0?'':'disabled'}>${art(id==='rawMeat'?'cookedMeat':'cookedFish')}${T('Cook')} · ${name(id)} (${s.inventory[id]})</button>`).join('')}</div><button class="text-button" data-home-action="${h.fire.lit?'extinguish':'relight'}" ${h.fire.fuel?'':'disabled'}>${T(h.fire.lit?'Put out the fire':'Light the fire')}</button></div>`;}
root.PECabin={camera,view,toRoom,toScreen,visible,fixtures,restore,home,begin,fade,hit,interact,tick,update,count,transfer,action,panel};if(typeof module!=='undefined')module.exports=root.PECabin;
