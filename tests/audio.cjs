...
for(const name of ['cast','lureSplash','fishNibble','fishHook','lineSnap','reelDrag','reelWind','catch']){const before=gains.length;audio.play(name,.3);assert.ok(gains.length>before,name+' emits fishing foley');}
s.cabinHome={inside:true,fire:{lit:false}};s.structures=[];s.weather='clear';s.playSeconds=400;engine.fishing={kind:'spinning',stage:'fight',held:false,tension:.7};
const beforeDrag=sources.length;audio.update(s,engine,false);assert.ok(sources.length>beforeDrag,'brake ticks under load');const afterDrag=sources.length;audio.update(s,engine,false);assert.equal(sources.length,afterDrag,'same clock does not stack reel sounds');s.playSeconds+=1;audio.update(s,engine,true);assert.equal(sources.length,afterDrag,'modal pauses reel sounds');
engine.fishing.held=false;engine.fishing.tension=0;audio.update(s,engine,false);assert.equal(sources.length,afterDrag,'slack idle reel is silent');engine.fishing.held=true;audio.update(s,engine,false);assert.ok(sources.length>afterDrag,'holding retrieves with quiet reel ticks');
audio.configure({soundOn:false});const mutedFishing=sources.length;for(const name of ['cast','lureSplash','fishNibble','fishHook','lineSnap','reelDrag','reelWind','catch'])audio.play(name);s.playSeconds+=1;audio.update(s,engine,false);assert.equal(sources.length,mutedFishing,'fishing sounds respect mute');
console.log('PASS fishing sound events, loaded drag/quiet retrieve, pause/idle gating and mute. No subjective listening claim.');
audio.configure({soundOn:true});engine.fishing={kind:'spinning',stage:'fight',held:true,tension:.95,dragRate:8};s.playSeconds+=2;const high=sources.length;audio.update(s,engine,false);assert.equal(sources.length-high,12,'high tension clearly ratchets even while the handle is held');
s.playSeconds+=2;engine.fishing.tension=.15;engine.fishing.dragRate=0;const low=sources.length;audio.update(s,engine,false);assert.equal(sources.length-low,2,'low-load retrieve remains quiet and sparse');const splash=gains.length;audio.play('fishSplash',.5);assert.ok(gains.length>splash);audio.configure({soundOn:false});const silentSplash=sources.length;audio.play('fishSplash');assert.equal(sources.length,silentSplash);
console.log('PASS loaded brake versus low-load retrieve cadence, airborne splash and mute.');
