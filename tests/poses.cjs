...
for(const id of ['pike','zander']){const a={species:id,weight:4,x:0,y:0,vx:25,vy:0,beat:0,effort:.8,jaw:0};const first=spinning.fishShape(a,40);a.beat=1.8;const second=spinning.fishShape(a,40);assert.notDeepEqual(first.tail,second.tail);assert.notDeepEqual(first.outline,second.outline);assert.deepEqual(first.top[0],second.top[0],'mouth does not drift as body bends');images.length=0;spinning.fish(renderer,a);assert.equal(images.length,0,'swimming fish never draw the static catch icon');const snapshot=JSON.stringify(a);spinning.fish(renderer,a);assert.equal(JSON.stringify(a),snapshot,'rendering never advances simulation');}
console.log('PASS animated fish spine/tail geometry, fixed mouth, no icon blit and pure rendering.');
